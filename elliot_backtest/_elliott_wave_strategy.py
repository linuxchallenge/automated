...
def identify_wave_structures(swings: list, config: StrategyConfig) -> list:
    ...
    Looks for the pattern: Low -> High -> Low -> High -> Low -> High
    (Wave1_start -> W1_end -> W2_end -> W3_end -> W4_end -> W5_end)

    For a long-only strategy in an uptrend.
    """
    structures = []

    # We need at least 4 swings to identify a partial structure
    # (W1_start_low, W1_end_high, W2_end_low = potential Wave 3 entry)
    for i in range(len(swings) - 2):
        ...
        # At this point we have a valid W1-W2 structure → potential Wave 3 entry
        ws = WaveStructure(
                ...
                if k + 2 < len(swings) and not swings[k + 2].is_high:
                    ...
                    if w4_end.price > w1_end.price:
                        if config.wave4_retrace_min <= wave4_retrace <= config.wave4_retrace_max:
                            ws.wave4_end = w4_end
                            ws.current_wave = WaveType.WAVE_5

                            # Look for Wave 5 end
                            if k + 3 < len(swings) and swings[k + 3].is_high:
                                ws.wave5_end = swings[k + 3]

        structures.append(ws)
...
def generate_signals(df: pd.DataFrame, structures: list,
    ...
    for ws in structures:
        ...
        # =====================================================================
        if ws.current_wave in (WaveType.WAVE_3, WaveType.WAVE_5):
