"""
===============================================================================
RUN ELLIOTT WAVE BACKTEST — NIFTY 200 (Last 6 Months)
===============================================================================
...
Setup:
    ...
    3. Run: python run_backtest_6m.py

Note:
    Downloads ~200 stocks × 6 months of daily data. First run takes
    1-2 minutes. Falls back to Nifty 50 if CSV not found.
===============================================================================
"""
...
def main():
    ...
    print("=" * 120)
    print(f"ELLIOTT WAVE LONG-ONLY STRATEGY — NIFTY 200 ({len(symbols)} stocks, 6 MONTHS: {start_date} to {end_date})")
    print("=" * 120)
    ...
    # ── Performance Summary ──────────────────────────────────────────────
    print(f"\n{'=' * 120}")
    print("BACKTEST PERFORMANCE SUMMARY (6 MONTHS)")
    print("=" * 120)
    ...
    if bt.skipped_signals_log:
        ...
        print(f"\n  Ignored Signals by Month:")
        skipped_df["month"] = pd.to_datetime(skipped_df["date"]).dt.to_period("M")
        monthly_skipped = skipped_df.groupby(["month", "reason"]).size().unstack(fill_value=0)
        print(monthly_skipped.to_string())
