...
class alligator_api(object):
    ...
    def compute_trend(self, last_df):
        ...
        cross_over = "no"
        try:
            max_alligator = max(last_df.iloc[-1][7], last_df.iloc[-1][8], last_df.iloc[-1][9])
            min_alligator = min(last_df.iloc[-1][7], last_df.iloc[-1][8], last_df.iloc[-1][9])

            if min_alligator > last_df.iloc[-1][4]:
                trend = "downtrend"

            if max_alligator < last_df.iloc[-1][4]:
                trend = "uptrend"

            if (last_df.iloc[-2][7] > last_df.iloc[-2][9]) and (last_df.iloc[-1][7] < last_df.iloc[-1][9]):
                cross_over = "bullish"
